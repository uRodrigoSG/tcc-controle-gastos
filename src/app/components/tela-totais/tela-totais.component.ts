...
export class TelaTotaisComponent implements OnInit {
  ...
  calcularTotais() {
    ...
    this.gastos.forEach((gasto) => {
      ...
      mapaCategoria.set(
        gasto.categoria,
        (mapaCategoria.get(gasto.categoria) || 0) + gasto.valor
      );
    ...
    this.totaisPorCategoria = Array.from(mapaCategoria.entries()).map(
      ([categoria, total]) => ({ categoria, total })
    );
    this.totaisPorItem = Array.from(mapaItem.entries()).map(
      ([item, total]) => ({ item, total })
    );
    this.totaisPorMes = Array.from(mapaMes.entries()).map(
      ([mesAno, total]) => ({ mesAno, total })
    );
